...
**L5（全国地図）完了。** 収録 2,805 件（Geoshape 2,808 行 → 重複 3 行を統合）、
46 都道府県、地形特徴 2,804 件、立地の埋め込みと二次元配置 2,805 件。
`/map/` で全件を地図に並べ、都道府県で絞り、名前で探し、古墳を選ぶと地形と
**立地の似た古墳（上位 10 件）** を出す。二次元配置の探索画面はこれから作る。
...
```bash
pnpm build            # → out/
pnpm smoke            # node harness/smoke.mjs。0 = 合格 / 1 = 不合格 / 2 = 前提不足 / 3 = 検品器が落ちた

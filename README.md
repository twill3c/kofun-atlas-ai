...
**L7（QA・公開）完了。** 収録 2,805 件（Geoshape 2,808 行 → 重複 3 行を統合）、
...
**本番: https://kofun-atlas-ai.vercel.app**（2026-09-15 公開・静的配信・`main` への push で本番に出る）。
...
| 国土地理院「淡色地図」 | 地図の背景（閲覧時に直接取得・同梱しない） | 国土地理院コンテンツ利用規約 |
| Wikidata | **V1.0 では未取り込み**（robots.txt が SPARQL を Disallow。人手スナップショットの手順のみ） | CC0 |
| 文化庁「国指定文化財等データベース」 | **V1.0 では未取り込み**（人手スナップショットの手順のみ） | 出典記載で利用可 |
| 全国遺跡報告総覧 | 使っていない | 個別確認 |
...
公開データ  →  Python パイプライン  →  GitHub  →  Vercel（静的）
                ...
                terrain features
                PyTorch → ONNX
                                        ブラウザで検索・絞り込み・類似度（配った埋め込みの内積）
```

学習はオフライン。本番はサーバ関数・DB・cron を持たない。`/api/health` もビルド時に書き出す静的な JSON である（`dynamic = "force-static"`）。
...
./.venv/Scripts/python.exe -m pip install -e ".[dev,validate]"   # validate は検査だけが読む依存(onnxruntime・scikit-learn)
```
...
検品器の陽性対照（層を隠す・データを空にする・本文色を漏らす・空白を注入する・名前の無いボタンや読めない文字を足す）は同じ実行の中で撃つ。
アクセシビリティ（言語・見出し・操作部品の名前・図の代替・コントラスト・焦点の輪）も 6 ページで測る。

本番を検品するときは URL を渡す。**最初に本番のデータと各ページの描画本文を手元の `out/` と突き合わせ、
違えば他を見ずに止める**（健やかさの検査は古い版にも通るため）。

```bash
SMOKE_BASE_URL=https://<本番のドメイン> node harness/smoke-run.mjs
```
